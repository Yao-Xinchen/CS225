 ...
 * @class SquareMaze
 *
 * @brief Each SquareMaze object represents a randomly-generated square maze and its solution.
 *
 * @note By "square maze" we mean a maze in which each cell is a square; the maze itself need not be a square.
 */
class SquareMaze
{
public:
    ...
    /**
     * @brief This uses your representation of the maze to determine whether it is possible to travel in the
     * given direction from the square at coordinates (x,y).
     *
     ...
     * that your maze is a tree that occupies the whole grid, and to verify your maze solution. So make sure that
     * this function works!
     *
     ...
     *
     * This function should be fast (constant time). You can assume that in grading we will not make your maze a
     * non-tree and then call one of the other member functions. setWall should not prevent cycles from occurring,
     * but should simply set a wall to be present or not present. Our tests will call setWall to copy a specific maze
     ...
     *
     * For each square on the bottom row (maximum y coordinate), there is a distance from the origin (i.e. the
     * top-left cell), which is defined as the length (measured as a number of steps) of the only path through
     ...
     * solveMaze() returns the winning path from the origin to the destination as a vector of integers, where each
     * integer represents the direction of a step, using the same encoding as in canTravel().
     *
     ...
     *
     * First, create a new PNG. Set the dimensions of the PNG to (width*10+1,height*10+1). where height and width were
     * the arguments to makeMaze. Blacken the entire topmost row and leftmost column of pixels, except the entrance
     * (1,0) through (9,0). For each square in the maze, call its maze coordinates (x,y). If the right wall exists,
     * then blacken the pixels with coordinates ((x+1)*10,y*10+k) for k from 0 to 10. If the bottom wall exists, then
     * blacken the pixels with coordinates (x*10+k, (y+1)*10) for k from 0 to 10.
     *
     * The resulting PNG will look like the sample image, except there will be no exit from the maze and the red line
     * will be missing.
     *
    ...
    /**
     * @brief This function calls drawMaze, then solveMaze; it modifies the PNG from drawMaze to show the solution
     * vector and the exit.
     *
     * Start at pixel (5,5). Each direction in the solution vector corresponds to a trail of 11 red pixels in the
     * given direction. If the first step is downward, color pixels (5,5) through (5,15) red. (Red is 0,1,0.5,1 in
     * HSLA). Then if the second step is right, color pixels (5,15) through (15,15) red. Then if the third step is up,
     * color pixels (15,15) through (15,5) red. Continue in this manner until you get to the end of the solution vector,
